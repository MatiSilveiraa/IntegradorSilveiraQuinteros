...
export default function ClasesPage() {
  ...
  const [diasAbiertos, setDiasAbiertos] =
    useState<Record<string, boolean>>({});

  const [menuClaseId, setMenuClaseId] =
  ...
  const [claseUbicacion, setClaseUbicacion] =
    useState<Clase | null>(null);

  const [claseEstado, setClaseEstado] =
    useState<Clase | null>(null);
  ...
  const clasesAgrupadas = useMemo(() => {
    ...
    clasesFiltradas.forEach((clase) => {
      ...
      const actuales = grupos.get(dia) ?? [];
      actuales.push(clase);
      grupos.set(dia, actuales);
    });
  ...
  useEffect(() => {
    ...
    DIAS_ORDENADOS.forEach((dia) => {
      if ((clasesAgrupadas.get(dia)?.length ?? 0) > 0) {
        inicial[dia] = true;
      }
    });
  ...
  const resumen = useMemo(() => {
    return {
      ...
      recurrentes: clases.filter(
        (clase) => clase.esFija,
      ).length,
      puntuales: clases.filter(
        (clase) => !clase.esFija,
      ).length,
    };
  }, [clases]);
  ...
  const abrirModalEstado = (clase: Clase) => {
    ...
    setNuevoEstado(
      obtenerValorEstado(clase.estado),
    );
    setMotivoEstado("");
  };
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-gray-300">
                Vista compacta organizada por día y
                horario, con acciones secundarias dentro
                de un único menú.
              </p>
            </div>
      ...
      <section className="w-full max-w-lg rounded-3xl border border-[#2d463b] bg-[#1a2b24] p-7 shadow-2xl">
        ...
        {(nuevoEstado === 0 ||
          ...
          <textarea
            ...
            value={motivo}
            onChange={(event) =>
              onMotivoChange(event.target.value)
            }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 px-4">
      <section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-[#2d463b] bg-[#1a2b24] p-7 shadow-2xl">
        ...
        <p className="mt-3 text-xs text-gray-500">
          Radio configurado:{" "}
          {clase.radioGeolocalizacion} metros
        </p>
      </section>
    </div>
  );
...
function compararClases(
  a: Clase,
  b: Clase,
) {
  const diaA =
    DIAS_ORDENADOS.indexOf(
      normalizarDia(a.diaSemana) as
        (typeof DIAS_ORDENADOS)[number],
    );

  const diaB =
    DIAS_ORDENADOS.indexOf(
      normalizarDia(b.diaSemana) as
        (typeof DIAS_ORDENADOS)[number],
    );
  ...
  return obtenerNombreGrupo(a).localeCompare(
    obtenerNombreGrupo(b),
    "es",
  );
}

...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await solicitarRecuperacion(emailNormalizado);
    ...
    <div className="min-h-screen bg-[#12201b] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg bg-[#1a2b24] border border-[#2d463b] rounded-3xl p-6 sm:p-8 shadow-2xl">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="text-white text-3xl font-bold mt-5">Recuperar contraseña</h1>

          <p className="text-gray-400 mt-3 leading-relaxed">
            Ingresá tu correo y te enviaremos un código para crear una nueva contraseña.
          </p>
        </div>
        ...
        {!enviado ? (
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <div>
              <label htmlFor="forgot-email" className="text-sm text-gray-300 block mb-2">
                Correo electrónico
              </label>
